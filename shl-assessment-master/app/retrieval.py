...
class CatalogIndex:
    ...
    def find_by_name(self, name: str) -> List[CatalogItem]:
        normalized_query = _normalize_search_name(name)
        exact = [i for i in self.items if _normalize_search_name(i.name) == normalized_query]
        if exact:
            return exact
        return [
            i
            for i in self.items
            if normalized_query in _normalize_search_name(i.name)
        ]
