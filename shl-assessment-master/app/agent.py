...
class Agent:
    ...
    def handle(self, messages: List[Message]) -> ChatResponse:
        ...
        if state.explicit_final_list or state.explicit_remove:
    ...
    def _handle_assessment_knowledge(self, state: ConversationState, messages: List[Message]) -> ChatResponse:
        ...
        comparison_targets = [target for target in state.comparison_targets if target and target.strip()]
        if comparison_targets:
            resolved_items = self._resolve_comparison_items(comparison_targets)
            if len(resolved_items) >= 2:
                return self._build_comparison_response(state, resolved_items[:2], query)
        ...
        if is_comparison and len(matched_items) >= 2:
            return self._build_comparison_response(state, matched_items[:2], query)
    ...
    def _build_comparison_response(self, state: ConversationState, items: List[CatalogItem], query: str) -> ChatResponse:
        if len(items) < 2:
            return ChatResponse(
                reply=(
                    "I could not find two matching SHL assessment entries in the catalog for that comparison request. "
                    "Please provide the exact assessment names or IDs you want to compare."
                ),
                state=state,
                end_of_conversation=True,
            )

        reply = self._format_assessment_comparison(items)
        recommendations = [self._build_catalog_recommendation(item) for item in items]
        return ChatResponse(reply=reply, recommendations=recommendations, state=state, end_of_conversation=True)
    ...
    def _copy_item_with_name(self, item: CatalogItem, name: str) -> CatalogItem:
        return CatalogItem(
            entity_id=item.entity_id,
            name=name,
            url=item.url,
    ...
    def _resolve_explicit_items(self, names: List[str]) -> List[CatalogItem]:
        ...
        def score_match(match_name: str, query_name: str) -> tuple[int, int, int]:
            score = 0
            normalized_match = self._normalize_lookup_name(match_name)
            normalized_query = self._normalize_lookup_name(query_name)
            if normalized_match == normalized_query:
                score -= 100
            if normalized_match.startswith(normalized_query):
                score -= 50
            if "report" in normalized_match and normalized_match != normalized_query:
                score += 20
            return (score, len(match_name), match_name.lower().count("report"))
        ...
        for name in names:
            ...
            if item and item.entity_id not in seen:
                items.append(self._copy_item_with_name(item, name))
                seen.add(item.entity_id)
                continue

            matches = self.index.find_by_name(name)
            if matches:
                matches.sort(key=lambda match: score_match(match.name, name))
                for match in matches:
                    if match.entity_id not in seen:
                        display_name = match.name
                        if self._normalize_lookup_name(match.name) == self._normalize_lookup_name(name):
                            display_name = name
                        items.append(self._copy_item_with_name(match, display_name))
                        seen.add(match.entity_id)
                        break
                continue

            search_results = self.index.search(name, top_k=5)
            for result in search_results:
                if result.item.entity_id not in seen:
                    items.append(self._copy_item_with_name(result.item, name))
    ...
    def _format_explicit_final_reply(self, state: ConversationState, recommendations: List[Recommendation], final_list: List[str]) -> str:
        ...
        lines = [
            ...
            "It is now aligned to your selected core assessments and avoids the previously suggested personality assessment.",
            "",
            "Final shortlist:",
        ]
        for idx, rec in enumerate(recommendations, start=1):
            lines.append(f"{idx}. {rec.name} — {rec.test_type} — {rec.duration} — {rec.url}")
        return "\n".join(lines)
